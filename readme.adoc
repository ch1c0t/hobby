...
class App
  ...
  map '/anatoly', to: C.new('Anatoly')
  map '/patricio', to: C.new('Patricio')
...
To perform mutation analysis do not run this:
...
Because it will eat all your memory and cpus.
Looks like there is an instance of
https://github.com/mbj/mutant#the-crash--stuck-problem-mri[this bug]
in `Hobby::Router#fill_builder`.
